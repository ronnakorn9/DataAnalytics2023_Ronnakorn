...
# prune the tree
pfitM2<- prune(fitM, cp=0.0180389) # from cptable??? adjust this to see the effect
# plot the pruned tree
plot(pfitM2, uniform=TRUE, main="Pruned Regression Tree for Mileage")
text(pfitM2, use.n=TRUE, all=TRUE, cex=.8)
### the split "Price >= 1.148e4" now disappear
